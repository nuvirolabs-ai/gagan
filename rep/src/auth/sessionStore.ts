...
export function createSessionStore(secure: SecureStorage, legacy: LegacyStorage, config: SessionStoreConfig) {
  ...
  return {
    async load(): Promise<SessionTokens | null> {
      ...
      try {
        ...
        if (typeof parsed.accessToken !== "string" || typeof parsed.refreshToken !== "string") throw new Error("invalid_session");
