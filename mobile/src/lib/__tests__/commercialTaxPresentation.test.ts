...
describe("commercial tax presentation", () => {
  it("marks a pending-tax quote as pre-tax and never presents GST as zero", () => {
    expect(commercialTaxPresentation({
      taxStatus: "PENDING",
      total: 3610,
      lines: [{ gstPercent: 0, gst: 0 }],
    })).toEqual({ pending: true, totalLabel: "Pre-tax total", gstLabel: "GST pending — final tax will be applied before invoicing." });
  });

  it("keeps a fully configured quote labelled as its grand total", () => {
