...
function collectionStatusLabel(status: string) {
  if (status === "confirming") return "Confirmation in progress";
    ...
    <div className="approval-layout">
      ...
      <section className="card approval-detail">
        {!selected ? <div className="empty-state">Select a collection to review its evidence.</div> : <>
          ...
          <p className="muted small">Collected by {selected.collectorName || "Collector unavailable"} · Status: {collectionStatusLabel(selected.status)}</p>
          ...
          <p>Notes: {selected.notes?.trim() || "Not supplied"}</p>
          {selected.invoiceScopeId && <p>Invoice {selected.invoiceScopeId} · Jain ₹{selected.jainAmount} · Padam ₹{selected.padamAmount}. Confirmation applies only to this invoice.</p>}
