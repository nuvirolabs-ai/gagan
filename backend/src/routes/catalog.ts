...
router.get("/catalog", requireAuth, async (req: AuthedRequest, res) => {
  ...
  const catalog = products.map((product) => ({
    ...
    variants: product.variants.map((v) => {
      return shapeVariant(v, resolve, inventoryByIdentity.get(inventoryLookupKey(product) ?? ""), req);
    }),
  }));
  ...
  const groups = groupCatalog(
    products.map((product) => ({
      ...
      variants: product.variants.map((v) =>
        shapeVariant(v, resolve, (() => {
          return inventoryByIdentity.get(inventoryLookupKey(product) ?? "");
        })(), req)
      ),
    }))
  );
  ...
  const [group] = groupCatalog(
    members.map((member) => ({
      ...
      variants: member.variants.map((v) =>
        shapeVariant(v, resolve, (() => {
          return inventoryByIdentity.get(inventoryLookupKey(member) ?? "");
        })(), req)
      ),
    }))
  );
