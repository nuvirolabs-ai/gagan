...
describe("Today board mapping", () => {
  ...
  it("locks the chairman mock KPIs", () => {
    ...
    expect(board.period.find((r) => r.id === "present")?.month).toEqual({ text: "94%", tone: "muted" });
    expect(board.tiles.map((tile) => tile.id)).toEqual(["present", "otif", "payments", "inventory"]);
    expect(board.tiles.map((tile) => tile.title).join(" ")).not.toMatch(/Orders|Collections|Fill rate|Blocked/);
